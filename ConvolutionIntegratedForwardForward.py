...
class FFDense(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp_layers = [keras.layers.Dense(size,
                                              use_bias = use_bias,
    ...
    def call(self, x):
        ...
        x_norm = x_norm + 1e-4
        x_dir = x / x_norm
        # print(np.shape(x_dir))
        for layer in self.mlp_layers:
            x_dir = layer(x_dir)
            x_dir = self.relu(x_dir)
            # print(np.shape(x_dir))
        return x_dir

    # The Forward-Forward pass
    def forward_forward(self, x_pos, x_neg):
        for i in range(self.num_epochs):
            ...
            gradients = tape.gradient(mean_loss, self.mlp_layers.trainable_weights)
            # print('Dense')
            # print(gradients)
            # print(self.mlp_layers.trainable_weights)
            self.optimizer.apply_gradients(zip(gradients, self.mlp_layers.trainable_weights))
            # print("Hello?")
        return (
...
class FFConv(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp_layers = []
        for size in units[:-1]:
            self.mlp_layers += [keras.layers.Conv2D(filters=size[0],
                                                    kernel_size=size[1],
                                                    use_bias=use_bias,
                                                    kernel_initializer=kernel_initializer,
                                                    bias_initializer=bias_initializer,
        ...
        self.reshape28=keras.layers.Reshape((28, 28, 1))
        self.loss_metric = loss_metric
        self.threshold = 1.5
        self.num_epochs = num_epochs
        self.print_dim=0
    ...
    def call(self, x):
        ...
        x_dir = x / x_norm
        # print(np.shape(x_dir))
        x_dir = self.reshape28(x_dir)
        # print(np.shape(x_dir))
        for layer in self.mlp_layers[:-1]:
            x_dir = layer(x_dir)
            # print('New:',np.shape(x_dir))
            x_dir = self.maxpooling(x_dir)
            # print(np.shape(x_dir))
            x_dir = self.relu(x_dir)
            # print(np.shape(x_dir))
        # print('After all these layers', np.shape(x_dir))
        x_dir = self.flatten(x_dir)
        layer = self.mlp_layers[-1]
        x_dir = layer(x_dir)
        x_dir = self.relu(x_dir)
        # print('Out of the Conv',np.shape(x_dir))
        return x_dir
...
class FFNetwork(keras.Model):
    ...
    def __init__(
        self,
        dims,
        layer_optimizer=tf.keras.optimizers.SGD(learning_rate=0.03, momentum=0.85),
...
if __name__=='__main__':
    ...
    # Model
    # Please mind the dimentions for FFConv to avoid numerical issues.
    model = FFNetwork(dims=[784, [[16, [3, 3]], [16, [3, 3]], 40], [20], [10]])
    model.compile(
        optimizer=tf.keras.optimizers.SGD(learning_rate=0.03, momentum=0.85),
        loss="mse",
        jit_compile=False,
